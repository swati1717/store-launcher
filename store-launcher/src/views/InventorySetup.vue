  ...
  <ion-page>
    <ion-header>
      <ion-toolbar class="header-toolbar">
        ...
        <ion-title>Inventory Setup</ion-title>
      </ion-toolbar>
    </ion-header>
    ...
    <ion-content class="ion-padding content-area">
      <div class="card">
        <div class="card-header" style="display: flex; align-items: center; gap: 16px; margin-bottom: 16px;">
          ...
          <h3 style="margin: 0; font-size: 1.5rem; font-weight: 600;">Bulk Inventory Setup</h3>
        </div>
        
        <p class="description">
          Upload a <b>.jsonl</b> file containing <code>ProductSetInput</code> GraphQL structures to rapidly set inventory levels across variants via Shopify's Staged Uploads API.
        </p>
        ...
        <div class="upload-controls" v-if="uploadStatus === 'IDLE' || uploadStatus === 'ERROR' || uploadStatus === 'COMPLETED'">
          ...
          <div class="template-section" style="background: rgba(0, 0, 0, 0.2); padding: 16px; border-radius: 8px; margin-bottom: 24px; margin-top: 16px;">
            ...
            <p style="font-size: 13px; color: #a0a0b0; margin: 0 0 12px 0; line-height: 1.5;">
              Your JSONL file should contain a single GraphQL <code>ProductSetInput</code> object per line, defining exactly <code>name: "available"</code> and <code>quantity</code> in <code>inventoryQuantities</code>.
            </p>
            <a href="/templates/default-inventory.jsonl" target="_blank" download class="ghost-link" style="display: inline-flex; align-items: center; gap: 6px; color: #667eea; text-decoration: none; font-size: 14px; font-weight: 600;">
        ...
        <div v-if="uploadStatus === 'COMPLETED' || uploadStatus === 'ERROR'" class="success-actions" style="margin-top: 24px;">
           ...
           <a v-if="uploadStatus === 'COMPLETED'" :href="`https://admin.shopify.com/store/${storeSlug}/products?selectedView=all`" target="_blank" rel="noopener noreferrer" class="ghost-btn icon-btn" style="display: flex; justify-content: center; margin-bottom: 12px;">
            <ion-icon :icon="openOutline"></ion-icon> View Inventory in Shopify Admin
          </a>
...
const loadDefaultDataset = async () => {
  ...
  try {
    ...
    statusMessage.value = `Location identified. Parsing product dataset...`;
    
    // Fetch the products template to extract handles and SKUs
